    ...
    if (!error) {
       ...
       [friendQuery whereKey:@"fbID" containedIn:friendIds];
        ...
        for (PFObject *dictionary in friendUsers) {
            for (NSDictionary *friend in friendObjects) {
                if ([[dictionary objectForKey:@"fbID"] isEqualToString:[NSString stringWithFormat:@"%@", [friend objectForKey:@"id"]]]) {
                    NSDictionary *userDictionary = [[NSDictionary alloc] initWithObjectsAndKeys:friend, kFacebookInfo, dictionary.objectId, kUserInfo , nil];
                    [friendUsersReturnArray addObject:userDictionary];
                    [friendObjects removeObject:friend];
                }
            }
        }
