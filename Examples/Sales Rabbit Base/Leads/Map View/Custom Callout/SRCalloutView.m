...
@interface UIView (SRFrameAdditions)
...
NSTimeInterval kSRCalloutViewRepositionDelayForUIScrollView = 1.0/3.0;

@interface SRCalloutView ()
...
@property (nonatomic, assign) SRCalloutArrowDirection currentArrowDirection;
@property (nonatomic, assign) BOOL popupCancelled;
@property (nonatomic, getter = isPresent) BOOL present; //To let know if callOut is present or not

@end

@implementation SRCalloutView

+ (SRCalloutView *)platformCalloutView {
    
    // if you haven't compiled SRClassicCalloutView into your app, then we can't possibly create an instance of it!
    if (!NSClassFromString(@"SRClassicCalloutView"))
        return [SRCalloutView new];
    
    // ok we have both - so choose the best one based on current platform
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1)
        return [SRCalloutView new]; // iOS 7+
    else
        return [NSClassFromString(@"SRClassicCalloutView") new];
}

- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        self.permittedArrowDirection = SRCalloutArrowDirectionDown;
        self.presentAnimation = SRCalloutAnimationBounce;
        self.dismissAnimation = SRCalloutAnimationFade;
...
- (SRCalloutBackgroundView *)backgroundView {
...
- (SRCalloutBackgroundView *)defaultBackgroundView {
    return [SRCalloutMaskedBackgroundView new];
}
...
- (void)presentCalloutFromRect:(CGRect)rect inLayer:(CALayer *)layer ofView:(UIView *)view constrainedToLayer:(CALayer *)constrainedLayer animated:(BOOL)animated {
    ...
    SRCalloutArrowDirection bestDirection = SRCalloutArrowDirectionDown;
    
    // we'll point it up though if that's the only option you gave us.
    if (self.permittedArrowDirection == SRCalloutArrowDirectionUp)
        bestDirection = SRCalloutArrowDirectionUp;
    ...
    if (self.permittedArrowDirection == SRCalloutArrowDirectionAny && topSpace < self.calloutHeight && bottomSpace > topSpace)
        bestDirection = SRCalloutArrowDirectionUp;
    ...
    CGFloat anchorY = self.calloutOffset.y + (bestDirection == SRCalloutArrowDirectionDown ? CGRectGetMinY(rect) : CGRectGetMaxY(rect));
    ...
    CGPoint calloutOrigin = {
        .x = calloutX + adjustX,
        .y = bestDirection == SRCalloutArrowDirectionDown ? (anchorY - self.calloutHeight) : anchorY
    };
...
- (CAAnimation *)animationWithType:(SRCalloutAnimation)type presenting:(BOOL)presenting {
    ...
    if (type == SRCalloutAnimationBounce) {
    ...
    else if (type == SRCalloutAnimationFade) {
        CABasicAnimation *fade = [CABasicAnimation animationWithKeyPath:@"opacity"];
    ...
    else if (type == SRCalloutAnimationStretch) {
...
- (void)layoutSubviews {
    ...
    CGFloat dy = self.currentArrowDirection == SRCalloutArrowDirectionUp ? TOP_ANCHOR_MARGIN : 0;
...
// import this known "private API" from SRCalloutBackgroundView
@interface SRCalloutBackgroundView (EmbeddedImages)
...
@interface SRCalloutMaskedBackgroundView ()
...
@implementation SRCalloutMaskedBackgroundView
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        if (!blackArrowImage) {
            blackArrowImage = [SRCalloutBackgroundView embeddedImageNamed:@"CalloutArrow"];
...
@implementation SRCalloutBackgroundView
...
@implementation UIView (SRFrameAdditions)
